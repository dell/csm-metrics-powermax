...
//
//go:generate mockgen -destination=mocks/storage_class_getter_mocks.go -package=mocks github.com/dell/csm-metrics-powermax/internal/k8s StorageClassGetter
...
func (f *StorageClassFinder) isMatch(class v1.StorageClass) bool {
	...
	for _, arrayID := range f.StorageArrayID {
		...
		storageArrayID, ok := class.Parameters["StorageArrayId"]

		if (storageArrayID == arrayID.ID) || (!ok && arrayID.IsDefault) {
			return true
		}
	}
